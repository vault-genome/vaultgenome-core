...
#
# The continuity drill with an Intel TDX Trust Domain as the standby: a
# model fine-tuned inside an AMD SEV-SNP VM fails over, under the
# operator's signed policy, to an attested TDX host — two TEE families,
# one policy, one project.
#
...
#              and the TDX standby; releases the last trustworthy genome's
#              key to the standby over mTLS across the VPC.
...
STANDBY="vg-fo-tdx-$LOW"
BUCKET="$PROJECT-vg-fotdx-$LOW"
BUILD="$(mktemp -d)"
...
echo "boot the AUTHORITY $AUTHORITY and the PRIMARY $PRIMARY (SEV-SNP) and the STANDBY $STANDBY (TDX)"
boot "$AUTHORITY" n2d-standard-4 SEV_SNP "AMD Milan" "$HERE/authority.sh" "${N2D_ZONES[@]}"; CREATED=1
boot "$PRIMARY" n2d-standard-8 SEV_SNP "AMD Milan" "$FAILOVER/primary.sh" "${N2D_ZONES[@]}"
boot "$STANDBY" c3-standard-4 TDX "" "$HERE/destination-tdx.sh" "${TDX_ZONES[@]}"
