...
# directly from /dev/sev-guest via ioctl on this Azure Confidential VM.
# Writes a 1184-byte raw report to ~/vg/evidence/snp-attestation-report.bin
# and a hex-dump preview to ~/vg/evidence/snp-attestation-report.txt.
#
# This is the RAW capture — for the full ARK→ASK→VCEK chain validation
# plus REPORT_DATA workload binding, run 02b-cryptographic-attestation.sh.
# For the Microsoft MAA JWT (Azure-specific 2nd verifier), run
# 02f-capture-maa-jwt.sh.
...
echo
echo "=== done. raw attestation captured ==="
echo "Next:"
echo "  ./scripts/02b-cryptographic-attestation.sh  # AMD chain validation"
