...
# Intel chain (oeutil) and MAA chain (JWT) evidence — Azure-specific
# dual-PKI structure.
...
echo "=== capture VM identity ==="
{
  ...
  echo
  echo "tee_backend:   azure-sgx (intel-sgx)"
  echo "intel_chain:   captured at attestation-validation/${VM_NAME}/{04-sgx-quote.bin, 06-verify-quote.txt, 07-dcap-verify.txt}"
