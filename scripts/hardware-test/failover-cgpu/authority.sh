...
# this chip, its escrow key sealed to this chip), whose one standby is a
# confidential GPU on Azure — acp-bootstrap on an NCC H100 v5, attesting
# as azure-cgpu, reached over the Internet with mTLS. It publishes its
# escrow key for the primary's sentinel, waits for the primary's identity
# and the destination's identity (the orchestrator hands the destination
# over), signs a policy pinning both, arms, and when the primary reports
# its compromise it releases the last trustworthy genome's key to the
# attested GPU. Reports and logs go to out/authority/; keys stay here.
set -u
...
OUT=/root/out; REPLICA=/root/replica; mkdir -p "$OUT" "$REPLICA" /root/vcek-cache /root/nras-cache; cd /root
...
gcs_get in/amd-genoa-cert_chain.pem /root/amd-genoa-cert_chain.pem
...
DPCR=$(python3 -c 'import json,sys;print(json.load(open(sys.argv[1])).get("pcr_digest_hex",""))' /root/destination.json)
...
step "sagvd failover config: the azure-cgpu destination in the registry (Genoa chain, NVIDIA's key set, the GPU policy, the boot pinned) and the primary's SEV-SNP anchors, the allow-list, the operator stop, the sealed escrow key, mTLS to the destination"
python3 - <<PY
import json
...
json.dump({"version":"failover-policy-v1","allowed":{"azure-cgpu":[DMEAS]}},open("/root/allow.json","w"))
...
step "sign the failover policy pinning the primary's chip and the GPU standby"
acpctl failover issue --key /root/operator.seed --kid operator-1 --serial 1 \
  ...
  --standby-kind azure-cgpu --standby-endpoint "$DEST_ENDPOINT" \
  --standby-measurement "$DMEAS" --heartbeat-timeout 120s --stopped-grace 60s --require-gate EQUIVALENT \
  --reason "failover drill: a CPU TEE on GCP fails over to an attested confidential GPU on Azure" --out /root/failover.json > "$OUT/failover-issue.txt"
...
step "arm: sagvd failover watches the replica; the standby is the GPU across the Internet"
...
for f in /root/vcek-cache/* /root/nras-cache/*; do [ -f "$f" ] && cp "$f" "$OUT/cache-$(basename "$f")"; done
