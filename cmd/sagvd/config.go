...
// for a REST API reachable beyond the local host (32 hex characters =
// 128 bits of entropy when generated by the deploy keygen).
const MinNonLoopbackTokenLen = 32

// isLoopbackBind reports whether a host:port listen address is
// reachable only from the local host. An empty host, "0.0.0.0", or
// "::" binds every interface and is therefore NOT loopback; any
// hostname other than "localhost" is treated as non-loopback because
// it may resolve to a routable address.
func isLoopbackBind(listenAddress string) bool {
...
func (c *Config) ResolveSecrets() error {
	...
	raw, err := os.ReadFile(c.HTTPAPI.BearerTokenFile)
	if err != nil {
		return fmt.Errorf("sagvd: read http_api.bearer_token_file: %w", err)
	}
	token := strings.TrimSpace(string(raw))
	if token == "" {
		return fmt.Errorf("sagvd: http_api.bearer_token_file %q is empty", c.HTTPAPI.BearerTokenFile)
	}
	if c.HTTPAPI.ListenAddress != "" && !isLoopbackBind(c.HTTPAPI.ListenAddress) &&
...
func (c Config) Validate() error {
	...
	if c.Vault.TLS.Enabled {
		...
		!isLoopbackBind(c.Vault.ListenAddress) {
	...
	if c.HTTPAPI.ListenAddress != "" {
		...
		} else if !isLoopbackBind(c.HTTPAPI.ListenAddress) {
