...
type VaultConfig struct {
	...
	// Enabled=false the listener is plain TCP — permitted only for
	// loopback / demo use.
...
type HTTPAPIConfig struct {
	...
	// BearerToken, if non-empty, is compared against the client-
	// supplied Authorization: Bearer <token> header on every write
	// endpoint. If empty the REST API accepts all callers — intended
	// for loopback demos only; operators must set it for any
	// non-loopback bind.
