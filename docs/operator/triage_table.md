...
| **LOW** | `POST /v1/jobs` returns 400, 403, 413 or 503 | sagvd REST API | Structural / Authority / Operational | Fix the request or the vault: `genome.bundle` must name a bundle in `genome.bundle_dir` (400 `genome_not_found`); its key file or escrow envelope must open it (403 `genome_key_invalid`); the model side must fit `runtime.max_payload_bytes` (413 `genome_too_large`); `deadline_seconds_from_now` ≤ `runtime.default_job_deadline_seconds`; 503 `gate_jobs_disabled` means `genome.bundle_dir` is not configured | None | `cmd/sagvd/http_api.go`, `genome_job.go` |
| **MEDIUM** | `sagvd gate refused the restored model` (`level: FAIL`, job `error.code: gate_failed`) | sagvd log / `GET /v1/jobs/{id}` | Operational | The worker restored a model that misses the sealed references at every door. Read `gate.attempts`; check the worker's base model against the genome's manifest (`vg_genome verify-base`), its torch build, and whether the genome was resealed. A `door_failed` reject names the door's own error | Worker fleet owner | ADR 0013, `cmd/sagvd/genome_job.go` |
| **HIGH** | `gate_output_invalid` (`level: ERROR`) on a job | sagvd log | Integrity | The signed answer is not an answer to the job — another genome's, or unreadable. Treat the worker as suspect (DR scenario 8) | SecOps | `cmd/sagvd/genome_job.go` |
